...
export default function ZeydooDashboard() {
  ...
  // Called when ZeydooSyncControl completes a successful sync
  const handleSyncComplete = useCallback(async () => {
    setSyncVersion(v => v + 1);
    // Revalidate latest stats so ZeydooLatestCard refreshes
    ...
    <div className="space-y-4">
      ...
      {activeTab === 'sync' && (
        <Section title="Zeydoo — Manual Sync">
          <ZeydooSyncControl onSyncComplete={handleSyncComplete} />
        </Section>
      )}
