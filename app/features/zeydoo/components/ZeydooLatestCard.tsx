...
import { useZeydooLatest } from '../hooks/useZeydooStats';

export default function ZeydooLatestCard({ onSync }: { onSync?: () => void }) {
  const { data, isLoading, error, refetch } = useZeydooLatest();
  ...
  if (status === 404 || (!data && !error)) {
    return (
      <div className="text-center py-8">
        <p className="text-sm text-gray-500 dark:text-gray-400 mb-3">No Zeydoo data yet.</p>
        <button
          onClick={onSync}
