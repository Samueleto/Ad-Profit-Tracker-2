...
import { useRollerAdsCountries } from '../hooks/useRollerAdsStats';

type SortKey = 'revenue' | 'revenueShare' | 'impressions' | 'clicks' | 'ctr';
...
const COLUMNS: { key: SortKey | 'country'; label: string }[] = [
  ...
  { key: 'revenueShare', label: 'Rev. Share' },
  { key: 'impressions', label: 'Impressions' },
  { key: 'clicks', label: 'Clicks' },
  { key: 'ctr', label: 'CTR' },
];

interface RollerAdsCountryBreakdownViewProps {
  dateFrom: string;
  dateTo: string;
...
export default function RollerAdsCountryBreakdownView({ dateFrom, dateTo, onDateFromChange, onDateToChange }: RollerAdsCountryBreakdownViewProps) {
  ...
  const sorted = [...rows].sort((a, b) => {
    if (sortKey === 'revenue') return sortAsc ? a.revenue - b.revenue : b.revenue - a.revenue;
                  ...
                  <td className="px-3 py-2.5">
                    <div className="flex items-center gap-2">
                      <div className="w-16 h-1.5 bg-gray-200 dark:bg-gray-700 rounded-full overflow-hidden">
                        <div className="h-full bg-blue-500 rounded-full" style={{ width: `${Math.min(row.revenueShare, 100)}%` }} />
                      </div>
