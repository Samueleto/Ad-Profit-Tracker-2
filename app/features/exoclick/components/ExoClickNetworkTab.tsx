...
export default function ExoClickNetworkTab() {
  ...
  const { refetch: refetchLatest } = useExoClickLatest();
  ...
  const handleTabChange = (tab: ExoTab) => {
    setActiveTab(tab);
    ...
    <div className="space-y-4">
      ...
      {activeTab === 'summary' && (
        <Section title="ExoClick — Latest Stats">
          <ExoClickLatestSummary onGoToSync={() => handleTabChange('sync')} />
        </Section>
      )}
      {activeTab === 'stats' && (
      ...
      {activeTab === 'sync' && (
        <Section title="ExoClick — Manual Sync">
          <ExoClickSyncPanel onSyncSuccess={handleSyncSuccess} />
        </Section>
      )}
      {activeTab === 'explorer' && (
