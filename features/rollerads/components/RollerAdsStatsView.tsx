...
import { useRollerAdsStats } from '../hooks/useRollerAdsStats';

interface RollerAdsStatsViewProps {
  dateFrom: string;
  dateTo: string;
...
export default function RollerAdsStatsView({ dateFrom, dateTo, onDateFromChange, onDateToChange }: RollerAdsStatsViewProps) {
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-gray-50 dark:bg-gray-800/50 border-b border-gray-200 dark:border-gray-700">
                {['Date', 'Revenue', 'Impressions', 'Clicks', 'CTR', 'CPM'].map(h => (
            ...
            <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
              {rows.map((row: Record<string, unknown>, i: number) => (
                <tr key={i} className="hover:bg-gray-50 dark:hover:bg-gray-800/50">
                  <td className="px-3 py-2 text-gray-700 dark:text-gray-300">{String(row.date ?? '—')}</td>
                  <td className="px-3 py-2 text-gray-700 dark:text-gray-300">${Number(row.revenue ?? 0).toFixed(4)}</td>
                  <td className="px-3 py-2 text-gray-700 dark:text-gray-300">{Number(row.impressions ?? 0).toLocaleString()}</td>
                  <td className="px-3 py-2 text-gray-700 dark:text-gray-300">{Number(row.clicks ?? 0).toLocaleString()}</td>
                  <td className="px-3 py-2 text-gray-700 dark:text-gray-300">{Number(row.ctr ?? 0).toFixed(2)}%</td>
                  <td className="px-3 py-2 text-gray-700 dark:text-gray-300">${Number(row.cpm ?? 0).toFixed(4)}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      {!isLoading && !error && !rangeError && totals && groupBy === 'day' && (
        <div className="pt-2 border-t border-gray-200 dark:border-gray-700">
          ...
          <div className="flex flex-wrap gap-4 text-sm">
            <span>Revenue: <strong>${Number(totals.revenue ?? 0).toFixed(4)}</strong></span>
