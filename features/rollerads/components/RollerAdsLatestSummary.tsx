...
export default function RollerAdsLatestSummary({ onGoToSync }: { onGoToSync?: () => void }) {
  const { data, isLoading, error, refetch } = useRollerAdsLatest();
  ...
  if (status === 404 || (!data && !error)) {
    return (
      <div className="text-center py-8">
        <p className="text-sm text-gray-500 dark:text-gray-400 mb-1">No RollerAds data yet.</p>
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
        <StatCell label="Revenue" value={latest?.revenue != null ? `$${Number(latest.revenue).toFixed(4)}` : '—'} />
